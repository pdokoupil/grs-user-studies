...
def get_supports(users_partial_lists, items, extended_rating_matrix, distance_matrix, users_viewed_item, k):
    ...
    print(f"Shapes=  {rel_supps.shape}, {div_supps.shape}, {nov_supps.shape}")
    return np.stack([rel_supps, div_supps, nov_supps])

class RLPropWrapper:
    def __init__(self, items, extended_rating_matrix, distance_matrix, users_viewed_item, normalization_factory, mandate_allocation, unseen_items_mask, cache_dir, discount_sequences):
    ...
    def _prepare_normalization(self):
        ...
        if self.cache_dir and os.path.exists(cache_path):
            ...
            num_users = self.extended_rating_matrix.shape[0]
            ...
            novelty_data_points = np.expand_dims(1.0 - self.users_viewed_item / num_users, axis=1)
    ...
    def __call__(self, k):
        ...
        for i in range(k):
            ...
            print(f"Predicting for i: {i + 1} out of: {k}")
